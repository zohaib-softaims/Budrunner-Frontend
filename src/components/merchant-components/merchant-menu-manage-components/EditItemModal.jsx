...
export const EditItemModal = ({ item, categoryId, modifiers, onClose, onSave }) => {
  ...
  const [pickupPrice, setPickupPrice] = useState(item.pickupPrice.toString());
  const [deliveryPrice, setDeliveryPrice] = useState(item.deliveryPrice.toString());
  const [usesModifierPricing, setUsesModifierPricing] = useState(item.usesModifierPricing);
  const [selectedModifier, setSelectedModifier] = useState(item.selectedModifier || "");
  const [options, setOptions] = useState(item.options || []);

  // Update form when item changes
  useEffect(() => {
    setName(item.name);
    setPickupPrice(item.pickupPrice.toString());
    setDeliveryPrice(item.deliveryPrice.toString());
    setUsesModifierPricing(item.usesModifierPricing);
    setSelectedModifier(item.selectedModifier || "");
  ...
  const handleModifierChange = (modifierId) => {
    ...
    if (modifier) {
      // If we're switching to a new modifier, initialize with its options
      if (modifierId !== item.selectedModifier) {
        setOptions(
      ...
      <div className="bg-white rounded-xl w-[480px] max-h-[85vh] overflow-y-auto shadow-xl">
        <div className="flex items-center justify-between p-6 border-b border-gray-100">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">Update your menu item</p>
          </div>
          <button 
            onClick={onClose} 
